...
### Content is addressed; nodes are not — OPEN

Content-addressing the *link* by `hash(source)` and freezing metadata pull
against each other. Two people upload the same source with different titles:
same id, whose title wins? Worse, the second uploader gets back a link owned by
a stranger's bucket, with someone else's title on it, that they cannot delete.

Recommendation: **separate the two.** Store content under `hash(source)` so the
bytes dedupe, and give each *node* its own id, its own metadata and its own
owning bucket, pointing at that content. Two uploads of the same source then
produce two links and one blob. Immutability is unaffected — both halves are
still write-once — and the surprising ownership case disappears.
...
- Whether link ids are content hashes or nodes pointing at content
  (recommendation above: nodes).
